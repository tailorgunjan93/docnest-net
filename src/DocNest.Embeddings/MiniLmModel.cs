...
/// <summary>
/// Locates (and optionally downloads) the all-MiniLM-L6-v2 ONNX model + vocab. The ~90 MB model is not
/// shipped in the package; <see cref="EnsureDownloadedAsync"/> fetches it from Hugging Face on first use
/// into a local cache. Opt-in (network), so offline/CI runs that haven't provisioned it skip cleanly.
/// </summary>
public static class MiniLmModel
{
    private const string ModelFileName = "all-MiniLM-L6-v2.onnx";
    private const string VocabFileName = "all-MiniLM-L6-v2.vocab.txt";
